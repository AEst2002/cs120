...
def QuickSelect(arr, i):
    ...
    for j in range(n):
        key = arr[j][0]
        if key > pivot:
            greaterArray.append(arr[j])
        elif key < pivot:
            lessArray.append(arr[j])
        else:
            equalArray.append(arr[j])
    lessLength = len(lessArray)
    equalLength = len(equalArray)
    if i < lessLength:
        return QuickSelect(lessArray, i)
    elif i >= lessLength + equalLength:
        return QuickSelect(greaterArray, i - lessLength - equalLength)
    else:
        return equalArray[0]
